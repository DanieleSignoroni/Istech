	...
	protected void otherActions(ClassADCollection cadc, ServletEnvironment se) throws ServletException, IOException {
		...
		if("INVIA_MODULA".equals(azione)) {
			try {
				...
				ErrorMessage em = YOrdAcqToModula.inviaAModulaMultple(objectKeys);
				if(em == null) {
					...
					String url = "/" + se.getServletPath() + "/it.thera.thip.acquisti.ordineAC.web.OrdineAcquistoGridActionAdapter?thClassName=OrdineAcquisto&ObjectKey="+URLEncoder.encode(key,"UTF-8")+"&thTarget=NEW&thAction=UPDATE_RIGHE";
